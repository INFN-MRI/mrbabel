...
from ._dicom2mrd import read_dicom_header, read_dicom_images, _get_unique_contrasts, IMTYPE_MAPS
...
def read_nifti_header(nii_data: np.ndarray, nii_head: dict) -> mrd.Header:
    ...
    # Complex images with separate magn/phase or real/imag
        ...
        if sum(image_types == 1) > 0:
            _nii_data[0] = np.zeros((sum(image_types == 1), *nii_data[0].shape), dtype=np.float32)
        if sum(image_types == 2) > 0:
            _nii_data[1] = np.zeros((sum(image_types == 2), *nii_data[0].shape), dtype=np.float32)
        if sum(image_types == 3) > 0:
            _nii_data[2] = np.zeros((sum(image_types == 3), *nii_data[0].shape), dtype=np.float32)
        if sum(image_types == 4) > 0:
            _nii_data[3] = np.zeros((sum(image_types == 4), *nii_data[0].shape), dtype=np.float32)
        ...
        if sum(image_types == 3) > 0 and sum(image_types == 4) > 0:
            ...
            if (_nii_data[1] > 2 * np.pi).any():
                mag = _nii_data[0].view()
                phase = _nii_data[1].view()
                phase = phase / 4096.0
                nii_data = _nii_data[0] * np.exp(1j * (2 * np.pi * _nii_data[1] / 4095))
            else:
                nii_data = _nii_data[0] * np.exp(1j * _nii_data[1])
    ...
    if (
        ...
        if "GE" in head.acquisition_system_information.system_vendor.upper():
            nii_data = np.fft.ifft(
                np.fft.fftshift(np.fft.fft(nii_data, axis=-3), axes=-3), axis=-3
            )
...
def read_nifti_image(nii_data: np.ndarray, nii_head: dict, head: mrd.Header) -> mrd.ImageArray:
    ...
    return (
        mrd.ImageArray(
            data=image_data.squeeze(), headers=image_head.squeeze(), meta=image_meta.squeeze()
        ),
        head,
    )
